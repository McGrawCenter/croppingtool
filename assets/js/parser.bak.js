...
class IIIFParser {
  ...
  *************************************/
  parsev2 (manifest) {
      ...
      var manifestObject = { "id":"", "type":"", "label":"", "summary":"", "metadata":"", "items":[]   }
  
      manifestObject.id = this.getFirstValue(manifest['@id']);
      manifestObject.type = "Manifest";
      manifestObject.label = this.getFirstValue(manifest.label);
      manifestObject.summary = this.getFirstValue(manifest.description);
      manifestObject.metadata = this.parseMetadata(manifest.metadata); 
      ...
      if(manifest.sequences) {
        ...
        for (const sequence of manifest.sequences) {
		  ...
		  // add this to the images array
                  manifestObject.items.push(imageobj);
